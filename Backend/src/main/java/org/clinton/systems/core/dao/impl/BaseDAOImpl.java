package org.clinton.systems.core.dao.impl;
...
public abstract class BaseDAOImpl<T> extends GenericDAOImpl<T, String> implements BaseDao<T> {
    ...
    public T save(T entity) {
        ...
        try {
            CustomSessionProvider customSessionProvider = ApplicationContextProvider.getBean(CustomSessionProvider.class);
    ...
    public T mergeBG(T obj) {
        sessionFactory.getCurrentSession().close();
        sessionFactory.getCurrentSession().beginTransaction();
        @SuppressWarnings("unchecked") T saved = (T) sessionFactory.getCurrentSession().merge(obj);
        sessionFactory.getCurrentSession().flush();
        sessionFactory.getCurrentSession().getTransaction().commit();
        sessionFactory.getCurrentSession().close();
        return saved;
